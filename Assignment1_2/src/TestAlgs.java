...
public class TestAlgs {
	 ...
	 */
	private static KeyRegistry loadKeyRegistry(String filePath)
	{
	 ...
	 */
	private static boolean isEncryptionAlg(Class c, HashMap<Class, HashMap<String,Method>> mappingAlgMeth)
	{
		boolean publConstr;
		try {
		...
		{
			Method m = methods[i];
			if (m.getName().startsWith("enc") && m.getParameterCount()==1 && m.getParameterTypes()[0]== String.class)
			{	
				encM = true;
				encMRef=m;
			}
			
			if (m.getName().startsWith("dec") && m.getParameterCount()==1 && m.getParameterTypes()[0]== String.class)
			{
				decM = true;
				decMRef= m;
			}
		}
		if(publConstr && encM && decM)
		{
			...
			mappingAlgMeth.put(c, mappingMethods);
	 ...
	 */
	private static void testAlg(String secretPath, Class alg, KeyRegistry kr, HashMap<Class, HashMap<String,Method>> mappingAlgMeth)
	{
		try {
			...
			while(scanner.hasNextLine())
			{
				...
				Method encr = mappingAlgMeth.get(alg).get("enc"), decr = mappingAlgMeth.get(alg).get("dec");
